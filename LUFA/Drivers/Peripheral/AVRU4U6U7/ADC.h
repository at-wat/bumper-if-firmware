...
#define __ADC_AVRU4U6U7_H__
	...
	/* Public Interface - May be used in end-application: */
		...
		/* Inline Functions: */
			 ...
			 */
			static inline void ADC_StartReading(const uint8_t MUXMask, const uint8_t option)
			{
				ADMUX = MUXMask & 0x1F | option;
				ADCSRB &= ~(1 << MUX5);
				if(MUXMask & 0x20) ADCSRB |= (1 << MUX5);
			 ...
			 *  \param[in] MUXMask  Mask comprising of an ADC channel mask, reference mask and adjustment mask
			 */
			static inline uint16_t ADC_GetChannelReading(const uint8_t MUXMask, const uint8_t option) ATTR_WARN_UNUSED_RESULT;
			static inline uint16_t ADC_GetChannelReading(const uint8_t MUXMask, const uint8_t option)
			{
				ADC_StartReading(MUXMask, option);
